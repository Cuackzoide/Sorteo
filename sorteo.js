...
window.addEventListener("DOMContentLoaded", () => {reproducirCancion()}) //Se activa al recargar pagina
...
iniciar.addEventListener("click", () => {                    //Boton iniciar sorteo
    const lista = crearLista();
    const pelucheGanador = elegirGanador(lista);
    ganador.innerHTML = `El peluche ganador es: <br> <strong id="` + //Mostramos la elección
    pelucheGanador.replaceAll(" ", "-") +                            //Asignamos el mismo ID para CSS.
     `">` + pelucheGanador + `</strong>`;
